...
var splitsterReducer = function splitsterReducer() {
  ...
  switch (action.type) {
    case 'splitster/INIT_SERVER':
      // $FlowFixMe
      return (0, _splitster.initSplitsterServer)(action.payload.config, action.payload.user, action.payload.userId, action.payload.def || state);
    case 'splitster/INIT_CLIENT':
      // $FlowFixMe
      return (0, _splitster.initSplitsterBrowser)(action.payload.config, action.payload.user, action.payload.userId, state);
